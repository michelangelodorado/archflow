...
export function DatabaseNode({ data, selected }: NodeProps) {
  ...
  return (
    <>
    <NodeHandles color="#4ade80" dualSides={(d.dualSides as Side[]) ?? []} />
    <div className={`px-4 py-3 rounded-lg border-2 bg-card shadow-sm min-w-[160px] min-h-[60px] h-full text-center flex flex-col justify-center items-center
      ${selected ? "border-primary ring-2 ring-primary/20" : "border-green-300"}`}
    >
      <NodeResizer isVisible={!!selected} minWidth={160} minHeight={60} />
      <div className="flex items-center gap-2 justify-center">
        <NodeIcon logo={logo} FallbackIcon={Database} className="w-4 h-4 text-green-500 flex-shrink-0" />
